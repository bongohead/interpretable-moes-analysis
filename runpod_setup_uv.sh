...
VENV_DIR="$HOME/.venv" # OR $PROJECT_DIR/.venv
KERNEL_NAME="moes-uv"
...
INSTALL_BUILD_TOOLS="0" # set to 1 if flash-attn needs to compile
...
uv pip install --python "$VENV_DIR/bin/python" \
  transformers==4.56.1 accelerate==1.10.1 triton==3.4.0 \
  tiktoken==0.11.0 blobfile==3.1.0 kernels==0.10.1 \
...
"$VENV_DIR/bin/python" -m ipykernel install --user --name "$KERNEL_NAME" --display-name "Python 3.12 (uv)"
