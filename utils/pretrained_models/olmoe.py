...
@torch.no_grad()
def run_olmoe_return_topk(model, input_ids, attention_mask):
    """
    Params:
...
@torch.no_grad()
def run_olmoe_with_ablation_return_topk(model, input_ids, attention_mask, layers_to_ablate = [], topk_to_ablate = [], renorm = False):
    """
    Params:
        @model: A model of class `OlmoeForCausalLM`.
        @input_ids: A B x N tensor of inputs IDs on the same device as `model`.
