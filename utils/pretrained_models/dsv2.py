...
- Model file: https://huggingface.co/deepseek-ai/DeepSeek-V2-Lite/blob/main/modeling_deepseek.py
- Note that old Deepseek-v2 model files are incompatible with transformers v4.54+; instead load the model with trust_remote_code = False
...
@torch.no_grad()
def run_dsv2_with_topk_ablation(model, input_ids, attention_mask, layers_to_ablate = [], topk_to_ablate = [], renorm = False, return_hidden_states = False):
    """
    Params:
        @model: A model of class `DeepseekV2ForCausalLM`.
        @input_ids: A B x N tensor of inputs IDs on the same device as `model`.
