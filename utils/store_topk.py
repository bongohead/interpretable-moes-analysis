...
@torch.no_grad()
def convert_topk_to_df(input_ids: torch.Tensor, all_topk_experts: list[torch.Tensor], all_topk_weights: list[torch.Tensor] = None) -> pd.DataFrame:
    """
    ...
    Returns:
        A dataframe at `sequence_ix` x `token_ix` x `layer_ix` x `topk_ix`, with columns:
    ...
    for layer_ix, layer_experts in enumerate(all_topk_experts):
        ...
        for token_pos in range(B * N):
            ...
            for k in range(top_k):
                ...
                if all_topk_weights is not None:
                    weights = all_topk_weights[layer_ix].cpu().numpy()
                    row["weight"] = weights[token_pos, k]
                else:
